...
function Verifica_Upload()
{
        ...
        function gera_fotos()
        {
                $diretorio = "../../../images/profissionais/";
				...
				reduz_imagem($this->arquivo['tmp_name'], 800, 600, $diretorio.$nome_foto);               
                                //passo o tamanho da thumbnail
                reduz_imagem($this->arquivo['tmp_name'], 175, 130, $diretorio.$nome_thumb); 
                echo "<span style=\"color: white; border: solid 1px; background: green;\">".$this->erro[$this->arquivo['error']]."</span>";
				$query = "UPDATE `fotos_profissionais` SET `id` = '$id', `link` = '$link', `nome_foto` = '$nome_foto', `nome_thumb` = '$nome_thumb' WHERE `fotos_profissionais`.`id` = ".$id.";";
