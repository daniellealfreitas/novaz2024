...
<body>
	...
	<section id="conteudo">
        <div id="wrap_menu_direito">
            <nav id="menu_direito">
                <ul>
                    <li class="odd"><a href="inclui_subcategoria_servico4.php">Incluir Sub Categoria</a></li>
                    <?php
						...
						{
						  echo '<li><a href="inclui_conteudo_servico4.php?id_subcategoria='.$linha['id'].'">'.$linha['nome'].'</a></li>';										  
						}
					?>	                    
                </ul>
            </nav>
        ...
        <article>
			<?php
				...
				$sql = 'SELECT * FROM categorias_servico4 WHERE id='.$id_subcategoria.'';
				$resultado = mysql_query($sql)
				or die ("Não foi possível realizar a consulta.");
				while ($linha=mysql_fetch_array($resultado))
				{
			...
			?>	
            <?php
				$verifica = mysql_query('SELECT * FROM conteudo_servicos4 WHERE id_subcategoria='.$id_subcategoria.''); 
				if(mysql_num_rows($verifica) > 0) { 
				$id_subcategoria=$_GET['id_subcategoria'];
				$sql = 'SELECT * FROM conteudo_servicos4 WHERE id_subcategoria='.$id_subcategoria.' LIMIT 0,1';
				$resultado = mysql_query($sql)
				or die ("Não foi possível realizar a consulta.");
				while ($linha=mysql_fetch_array($resultado))
					  {
						  //Formulário para editar							
						  echo '<form method="post" action="../../acoes/edita/edita_conteudo_servico4.php?id_subcategoria='.$id_subcategoria.'" enctype="multipart/form-data">';	
						  ...
						  echo '<form method="post"  action="../../acoes/inclui/inclui_conteudo_servico4.php?id_subcategoria='.$id_subcategoria.'" enctype="multipart/form-data">';						  
						  echo '<label>Texto</label><br>';	
						  echo '<textarea cols="35" rows="10" name="texto">'.$linha['texto'].'</textarea><br>';
						  echo '<input type="submit" value="Enviar">';
						  echo '</form>';				
        ...
        <article>
        	<h2>Edita Foto</h2>
            <?php
				$verifica = mysql_query('SELECT * FROM  `fotos_conteudo_servicos4` WHERE id_subcategoria='.$id_subcategoria.''); 
				if(mysql_num_rows($verifica) > 0) {
				$id_subcategoria=$_GET['id_subcategoria']; 
				$sql = 'SELECT * FROM  `fotos_conteudo_servicos4` WHERE id_subcategoria='.$id_subcategoria.'';
				$resultado = mysql_query($sql)
				or die ("Não foi possível realizar a consulta.");
				while ($linha=mysql_fetch_array($resultado))
					  {
						  //Formulário para editar
						  echo '<form method="post"  action="../../acoes/edita/edita_upload_foto_conteudo_servico4.php?id_subcategoria='.$id_subcategoria.'" enctype="multipart/form-data">';
						  ...
						  //Formulário para incluir
						  echo '<form method="post"  action="../../acoes/inclui/upload_foto_conteudo_servico4.php?id_subcategoria='.$id_subcategoria.'" enctype="multipart/form-data">';
						  echo '<label>Escolha a foto: (O tamanho ideal é 160x160px)</label><br>  ';
						  echo '<input type="file" name="arquivo"><br>';
						  echo '<input type="submit" value="Enviar">  ';
						  echo '</form>';
